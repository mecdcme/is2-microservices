...
public class DataSetDTO {
    ...
    public static class Builder {
        ...
        public Builder files(List<DatasetFileDTO> files) {
            this.files = files;
            return this;
        }

        public Builder filesFromEntity(List<DatasetFile> files) {
            this.files = new ArrayList<>(files.size());
            files.forEach(x -> this.files.add(new DatasetFileDTO.Builder().fromEntity(x).build()));
            return this;
        }

        public Builder numRighe(Integer numRighe) {
            this.numRighe = numRighe;
            return this;
        }

        public DataSetDTO build() {
            return new DataSetDTO(colonne, idfile, variabiliSum, dfile, files, numRighe);
        }
    }
}
